...
**25 星空彼岸花** 与 **26 怦然心动** 已支持创作面板、JSON 配方和当前画面 PNG。可以从预设开始，用滑块调整画面，再保存这组参数，交给另一位同学或下次重新打开。运行作品、保存配方和巡展只需要 Python 标准库中的 Turtle / Tkinter；可选 PNG 导出需另装 Pillow，首批支持 Windows。
...
PNG 保留作品题字，去除窗口边框、舞台工具说明、创作面板与巡展条。按当前窗口绘图区像素保存，以完成提示为准；它适合分享眼前这一帧，尚不支持指定比例、矢量或按目标大尺寸重新绘制。窗口缩得过小或最小化时可能无法导出，恢复窗口后再试。[巡展接管规则、PNG 用途与完整限制 →](EXHIBITION_GUIDE.md)
...
当前画面 PNG 与 25、26、27 巡展已接入；不同画幅、目标大尺寸重绘、短片和浏览器试玩继续按 [升级路线](ROADMAP.md) 分阶段实现。欢迎提交使用反馈或分享配方，并附上作品编号、项目版本和想要的画面效果。
