...
| 横屏 / 竖屏 / 方形构图 | 25 星空彼岸花、26 怦然心动 | 创作面板选择画幅，主体、背景与题字随画幅布局 |
...
1. 打开 **25 星空彼岸花** 或 **26 怦然心动**，按 **E** 或点击作品右上角 **创作面板**，选择原始画幅、横屏 16:9、竖屏 9:16 或方形 1:1。预设配色会保留所选画幅。
2. 调整参数并等待喜欢的画面，也可用空格停在想保存的位置。在面板中点击 **导出当前画面 PNG…**。
...
默认使用原始画幅，旧版配方载入后也保持原始画幅。横屏适合投影和幻灯片，竖屏适合手机展示，方形适合作品卡。切换画幅后，两件作品会重新安排主体、背景和题字。更多参数与新版、旧版示例见 [创作指南](CREATION_GUIDE.md)。

![两件作品在横屏、竖屏和方形画幅中的构图](assets/aspect-compositions.png)

选择横屏、竖屏或方形时，可在面板勾选 **显示 6% 题字安全区（仅预览）**，查看距四边各 6% 的参考线。原始画幅不显示参考线。开关不保存到 JSON 配方，参考线也不进入 PNG；捕获时会临时隐藏参考线，结束后恢复预览。

## PNG 保存什么

PNG 保存**当前画幅中的作品**，保留作品题字，去除系统窗口边框、Canvas 边框、画幅外留白、舞台工具说明、巡展控制条和安全区参考线。覆盖在作品上方的创作面板也不进入图片。

![通过创作面板实际导出的彼岸花，990 × 550 像素](assets/export-lily.png)

横屏、竖屏和方形的导出尺寸严格保持所选的 **16:9、9:16 或 1:1** 比例；原始画幅沿用原有绘图区范围。图片按当前窗口中的实际像素保存，**以完成提示的实际尺寸为准**。窗口尺寸、Tk 客户区和边框会影响像素数量，不能把窗口标称尺寸直接当作图片尺寸。

PNG 适合当前画面的分享和展示记录。画幅选择不会按目标大尺寸重新绘制或自动提高分辨率；目标大尺寸重绘、矢量与短片导出尚未提供，后续计划见 [升级路线](ROADMAP.md)。
...
三种画幅已在两件作品、760 × 580 与 1100 × 900 两种窗口下实际检查，共 12 组。较大窗口分别导出横屏 1040 × 585、竖屏 396 × 704、方形 704 × 704；最小窗口分别为 720 × 405、243 × 432、444 × 444。题字实际边界位于 6% 参考区内且互不重叠，开关参考线不改变 PNG 像素，切换配色预设保留画幅，暂停时导出保留动画时间。以上为本机结果，见 [composition-m2.json](benchmarks/composition-m2.json)。
...
此前原始画幅在 Windows、Python 3.12.4、Tk 8.6.14 上，两款作品的 1000 × 720 窗口导出为 990 × 550，760 × 580 窗口导出为 750 × 417；这些是本机结果，实际以面板提示为准。面板遮挡前后的导出像素一致，取消保存、元数据写入和暂停状态保留也已检查。混合 DPI 多显示器、远程桌面与锁屏环境尚未实测；macOS / Linux 暂不开放 PNG 导出入口能力。
