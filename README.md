...
```bash
python run.py --list       # 列出全部 24 款作品与编号
...
![互动展品运行片段](docs/assets/showcase.gif)
...
这 10 款作品共用操作：**空格暂停 · R 重来 · H 隐藏说明 · F11 全屏 · Esc 退出作品**。从画廊启动时，退出后可继续选下一款。[详细操作与展示顺序 →](社团展示/README.md)
...
一个容易上手的顺序：**烟花开场 → 四季树讲递归 → 万花筒请同学画一笔 → 弹球挑战 → 打开原作源码改一个参数**。按 H 收起说明、F11 全屏，适合投影；想讲解时按空格暂停画面。

从 [xin.py](xin.py) 的爱心填色开始，或把 [分形树.py](分形树.py) 与 [四季分形树](社团展示/06_四季分形树.py) 放在一起看：同一个递归想法，可以逐步长出颜色、风和交互。

## 继续创作

- [创作指南](docs/CREATIVE_GUIDE.md)：24 款作品的代码入口、原作操作和几个可以立刻动手的改法。
