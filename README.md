...
python run.py --tour 25,26,27 --seconds 30  # 三件作品循环巡展，每件 30 秒
...
这 14 款作品共用操作：**空格暂停 · R 重来 · H 隐藏说明 · F11 全屏 · Esc 退出作品**。25、26 的 R 会保留当前创作参数重新播放。从画廊启动时，退出后可继续选下一款。[详细操作与展示顺序 →](社团展示/README.md)

### 调出自己的彼岸花与粒子爱心

在 **25 星空彼岸花** 或 **26 怦然心动** 中按 **E**，或点击画面右上角的 **创作面板**：选一套预设，拖动滑块改变花瓣卷曲、微风、星空密度或爱心大小与粒子数量。点击“换一幅”探索新构图，满意后保存 JSON 配方，下次在同一作品中载入。

![星空彼岸花与可实时调参的创作面板](docs/assets/creator.png)

配方保存参数与构图种子，载入后从头播放；动画进度和点击过程不在其中。面板里的“恢复默认”与“重播作品”分别用于回到初始设置和重播当前创作。[创作步骤、示例配方与实测结果 →](docs/CREATION_GUIDE.md)
...
无人操作时，可从桌面画廊设置 **25 → 26 → 27** 自动巡展，调整顺序和每件停留时间。点击或按普通操作键后暂停切换，**P** 继续巡展，**PgDn** 下一件，**Esc** 结束。[巡展控制与 PNG 导出指南 →](docs/EXHIBITION_GUIDE.md)
